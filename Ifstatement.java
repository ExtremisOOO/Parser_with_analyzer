public class Ifstatement  extends DataBase{
    private int number1,number2;
    private int operatorIndex = 0;
    public boolean getIsPositiveIfStatement(String expression) {
    ...
    private String[] findOperatorIndex(String expression){
        ...
        output[0] = expression.substring(3,operatorIndex);
    ...
    private boolean analyzeSyntaxExpression(String expression){
        if(!(expression.charAt(3)=='(' || expression.endsWith(")"))){
            return false;
        }else{
            for (int j = 3; j < expression.length()-1; j++) {
